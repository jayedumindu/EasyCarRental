...
//    @Query(value = "select * from Driver LEFT JOIN Booking ON Driver.username = Booking.driver_username WHERE Booking.driver_username=NULL ORDER BY RAND() LIMIT 1",nativeQuery = true)
    @Query(value = "select * from Driver WHERE Driver.username NOT IN (select Booking.driver_username from Booking) OR Driver.username NOT IN (select Booking.driver_username from Booking where :date1 between Booking.currenDateTime and Booking.dueDateTime) AND Driver.username NOT IN (select Booking.driver_username from Booking where :date2 between Booking.currenDateTime and Booking.dueDateTime) ORDER BY RAND() LIMIT 1",nativeQuery = true)
